function [querydata,querylabel,traindata,trainlabel]...
    ...
    for Sel_i = 1:numofcom  
        Prediction = committees{Sel_i}(querydata);      % 第Sel_i个分类器对querydata进行预测
                                                        % 将第Sel_i个分类器的预测结果存放在Predict_cell中
        Predict_cell{Sel_i} = Prediction;
        count=vec2ind(compet(Prediction))';             % 将分类器的预测结果以一列数的形式表示
        predictresult(:,Sel_i) = count;                 % 将第Sel_i个分类器的结果放在predictresult的第Sel_i列中
    end
    ...
    for VE_m = 1:dataclass
        VE_V(:,VE_m) = sum((predictresult == VE_m),2);  % 统计投票结果中认为样本属于某一类的委员会成员的个数。
    end
    temp_VE = sum((VE_V./VE_K) .* log2(VE_V / VE_K + eps ), 2);     % 公式中的 sum{V(c,e)/K  *  log(V(c,e)/K)};
    VE = (-1 / log2(min(VE_K,dataclass)) * temp_VE)';               % 计算后转置成一行。
...
function divergence = KLdivergence(P,Q)
    divergence = sum(P.* log2(P./(Q+eps) + eps));
end